...
// attempt. `commit` alone would repeat across the nightly scheduled rebuild of
// an unchanged HEAD, and GITHUB_RUN_ID alone would repeat across a "re-run all
// jobs" -- either would let that check pass against the previous deployment.
// GITHUB_RUN_ATTEMPT is the part that distinguishes re-runs. `commit` stays
...
const run = `${process.env.GITHUB_RUN_ID ?? 'unknown'}-${process.env.GITHUB_RUN_ATTEMPT ?? '0'}`;
